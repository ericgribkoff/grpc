...
argp.add_argument(
    ...
    default=False,
    action='store_true',
    help='Continue with test even when an error occurs during setup. Intended for '
...
def test_backends_restart(compute, project, zone, instance_names, num_rpcs, stats_timeout_sec):
  ...
  for instance in instance_names:
      if abs(stats.rpcs_by_peer[instance] - new_stats.rpcs_by_peer[instance]) > 1:
        raise Exception('outside of threshold for ', instance, stats.rpcs_by_peer[instance], new_stats.rpcs_by_peer[instance])
...
nohup build/install/grpc-interop-testing/bin/xds-test-server --port=%d 1>/dev/null &"""
  ...
  result = compute.instanceTemplates().insert(
...
def create_instance_group(compute, name, size, grpc_port, template_url, project,
  ...
  result = compute.instanceGroupManagers().insert(
...
def create_health_check(compute, name, project):
  config = {'name': name, 'type': 'TCP', 'tcpHealthCheck': {'portName': 'grpc'}}
  result = compute.healthChecks().insert(project=project, body=config).execute()
  wait_for_global_operation(compute, project, result['name'])
  return result['targetLink']
...
def delete_url_map(compute, project, url_map):
  try:
    result = compute.urlMaps().delete(project=project, urlMap=url_map).execute()
    wait_for_global_operation(compute, project, result['name'])
  except googleapiclient.errors.HttpError as http_error:
    logger.info('Delete failed: %s', http_error)
...
def delete_firewall(compute, project, firewall_rule):
  try:
    result = compute.firewalls().delete(
        project=project, firewall=firewall_rule).execute()
    wait_for_global_operation(compute, project, result['name'])
  except googleapiclient.errors.HttpError as http_error:
    logger.info('Delete failed: %s', http_error)
...
def delete_instance_group(compute, project, zone, instance_group):
  try:
    ...
    wait_for_zone_operation(
        compute, project, ZONE, result['name'], timeout_sec=timeout_sec)
  except googleapiclient.errors.HttpError as http_error:
    logger.info('Delete failed: %s', http_error)
...
def start_xds_client():
  ...
  with tempfile.NamedTemporaryFile(delete=False) as bootstrap_file:
    bootstrap_file.write(
        BOOTSTRAP_TEMPLATE.format(node_id=socket.gethostname()).encode('utf-8'))
    bootstrap_path = bootstrap_file.name

  client_process = subprocess.Popen(
      shlex.split(cmd), env=dict(os.environ, GRPC_XDS_BOOTSTRAP=bootstrap_path))
...
try:
  ...
  try:
    ...
    health_check_url = create_health_check(compute, HEALTH_CHECK_NAME,
                                           PROJECT_ID)
    create_health_check_firewall_rule(compute, FIREWALL_RULE_NAME, PROJECT_ID)
    backend_service_url = create_backend_service(compute, BACKEND_SERVICE_NAME,
                                                 instance_group_url,
                                                 health_check_url, PROJECT_ID)
    url_map_url = create_url_map(compute, URL_MAP_NAME, backend_service_url,
                                 SERVICE_HOST, PROJECT_ID)
    target_http_proxy_url = create_target_http_proxy(compute, TARGET_PROXY_NAME,
                                                     url_map_url, PROJECT_ID)
    create_global_forwarding_rule(compute, FORWARDING_RULE_NAME, SERVICE_PORT,
                                  target_http_proxy_url, PROJECT_ID)
  except googleapiclient.errors.HttpError as http_error:
  ...
  if TEST_CASE == 'all':
    ...
    test_backends_restart(compute, PROJECT_ID, ZONE, backends, NUM_TEST_RPCS, WAIT_FOR_STATS_SEC)
  elif TEST_CASE == 'change_backend_service':
    test_change_backend_service(backends, NUM_TEST_RPCS, WAIT_FOR_STATS_SEC)
  elif TEST_CASE == 'new_instance_group_receives_traffic':
    test_new_instance_group_receives_traffic(backends, NUM_TEST_RPCS,
                                             WAIT_FOR_STATS_SEC)
  elif TEST_CASE == 'ping_pong':
    test_ping_pong(backends, NUM_TEST_RPCS, WAIT_FOR_STATS_SEC)
  elif TEST_CASE == 'remove_instance_group':
    test_remove_instance_group(backends, NUM_TEST_RPCS, WAIT_FOR_STATS_SEC)
    ...
    test_secondary_locality_gets_requests_on_primary_failure(
        backends, NUM_TEST_RPCS, WAIT_FOR_STATS_SEC)
