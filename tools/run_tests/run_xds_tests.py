    ...
    while time.time() - start_time <= timeout_sec:
        ...
        for instance_name in instance_names:
            try:
                ...
                logger.info('health check response from %s: %s', instance_name, status)
            except grpc.RpcError as rpc_error:
                logger.info('checking health service of %s failed: %s', instance_name, rpc_error)
        time.sleep(5)
