...
void childReadsInheritedCall() {
  ...
  HelloRequest request;
  request.set_name("on inherited call");
  if (!stream->Write(request)) {
    std::cout << "Error writing" << std::endl;
  }
  HelloReply reply;
...
int main(int argc, char** argv) {
  ...
  if (test_case == "keep_alive") {
    ...
    midCallRecovery();
  } else if (test_case == "child_reads_inherited_call") {
    std::cout << "Running test case child_reads_inherited_call" << std::endl;
    childReadsInheritedCall();
